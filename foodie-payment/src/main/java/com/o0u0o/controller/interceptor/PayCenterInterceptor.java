...
public class PayCenterInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, 
		...
		if (StringUtils.isNotBlank(imoocUserId) && StringUtils.isNotBlank(password)) {
			...
			if (user == null) {
				returnErrorResponse(response, new IJsonResult().errorTokenMsg("用户id或密码不正确！"));
				return false;
			}
			...
			if (days < 0) {
				returnErrorResponse(response, new IJsonResult().errorTokenMsg("该账户授权访问日期已失效！"));
				return false;
			}
			...
			returnErrorResponse(response, new IJsonResult().errorTokenMsg("请在header中携带支付中心所需的用户id以及密码！"));
			return false;
		}
