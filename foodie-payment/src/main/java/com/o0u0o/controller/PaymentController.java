...
import com.o0u0o.enums.PaymentStatus;
import com.o0u0o.pojo.Orders;
import com.o0u0o.pojo.bo.MerchantOrdersBO;
import com.o0u0o.pojo.vo.PaymentInfoVO;
import com.o0u0o.resource.AliPayResource;
import com.o0u0o.resource.WXPayResource;
import com.o0u0o.service.PaymentOrderService;
import com.o0u0o.utils.CurrencyUtils;
import com.o0u0o.utils.IJsonResult;
import com.o0u0o.utils.RedisOperator;
import com.o0u0o.wx.entity.PreOrderResult;
import com.o0u0o.wx.service.WxOrderService;
...
public class PaymentController {
	...
	/**
	 * 接受商户订单信息，保存到自己的数据库(支付中心的数据库)
	 */
	@PostMapping("/createMerchantOrder")
	public IJsonResult createMerchantOrder(@RequestBody MerchantOrdersBO merchantOrdersBO, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (StringUtils.isBlank(merchantOrderId)) {
			return IJsonResult.errorMsg("参数[orderId]不能为空");
		}
		if (StringUtils.isBlank(merchantUserId)) {
			return IJsonResult.errorMsg("参数[userId]不能为空");
		}
		if (amount == null || amount < 1) {
			return IJsonResult.errorMsg("参数[realPayAmount]不能为空并且不能小于1");
		}
		if (payMethod == null) {
			return IJsonResult.errorMsg("参数[payMethod]不能为空并且不能小于1");
		}
		if (payMethod != PayMethod.WEIXIN.type && payMethod != PayMethod.ALIPAY.type) {
			return IJsonResult.errorMsg("参数[payMethod]目前只支持微信支付或支付宝支付");
		}
		if (StringUtils.isBlank(returnUrl)) {
			return IJsonResult.errorMsg("参数[returnUrl]不能为空");
		}
		...
		try {
			...
			e.printStackTrace();
			IJsonResult.errorException(e.getMessage());
		}

		if (isSuccess) {
			return IJsonResult.ok("商户订单创建成功！");
		} else {
			return IJsonResult.errorMsg("商户订单创建失败，请重试...");
		}
	}
	...
	public IJsonResult getPaymentCenterOrderInfo(String merchantOrderId, String merchantUserId) {

		if (StringUtils.isBlank(merchantOrderId) || StringUtils.isBlank(merchantUserId)) {
			return IJsonResult.errorMsg("查询参数不能为空！");
		}

		Orders orderInfo = paymentOrderService.queryOrderInfo(merchantUserId, merchantOrderId);

		return IJsonResult.ok(orderInfo);
	}
...
//	@GetMapping(value="/getWXPayQRCode")
	...
	public IJsonResult getWXPayQRCode(String merchantOrderId, String merchantUserId) throws Exception{
...
//				String total_fee = "1";	// 测试用 1分钱
			...
			return IJsonResult.ok(paymentInfoVO);
		} else {
			return IJsonResult.errorMsg("该订单不存在，或已经支付");
		}
	}
	...
	public IJsonResult goAlipay(String merchantOrderId, String merchantUserId) throws Exception{
...
//    	String total_amount = "0.01";	// 测试用 1分钱
		...
		return IJsonResult.ok(alipayForm);
	}
